...
namespace SchematicNeo4j.Tests.Indexes
{
    public class Index_Create_Tests : IDisposable
    {
        ...
        private readonly Index testIndex = new Index(label: "TestIndexCreate", new string[] { "Prop1", "Prop2" });
            ...
            using (var session = driver.Session(AccessMode.Write))
            {
                session.WriteTransaction(tx => {
                    if (testIndex.Exists(tx))
                        tx.Run("DROP INDEX ON :TestIndexCreate(Prop1,Prop2)");
                });
            }
        }
        ...
        private void CreateIndexForTest()
        {
            using (ISession session = driver.Session(AccessMode.Write))
            {
                session.WriteTransaction(tx => tx.Run("CREATE INDEX ON :TestIndexCreate(Prop1,Prop2)"));
            }
        }
